...
var loadController = function(){
	try {
		...
		if (e instanceof KollusPostMessageExcpetion && e.code === -99) {
			console.log(e);
		}
	}
}
$(document).ready(function() {
	...
	$('#sb_library').change(function() {
		var libraryKey = $('#sb_library option:selected').val();
		var txtLibrary = $('#sb_library option:selected').text();
		var start = txtLibrary.indexOf('(');
		var end = txtLibrary.indexOf(')', start + 1);
		var cntLibrary = txtLibrary.substring(start + 1, end);
		var url = 'media/library';
		var data = {};
		data.accessToken = $('#tb_accessToken').val();
		data.accountKey = $('#tb_accountKey').val();
		data.secretKey = $('#tb_secretKey').val();
		data.userKey = $('#tb_userKey').val();
		data.page = cntLibrary;
		if (libraryKey != '' && libraryKey != undefined) {
			url += '/' + libraryKey;
		}
		var list_media = $(this).parent().find('ul');
		$.ajax({
			url : url,
			type : 'get',
			data : data,
	...
	$('#sb_channel').change(function() {
		...
		var url = 'media/channel';
		var data = {};
		data.accessToken = $('#tb_accessToken').val();
		data.accountKey = $('#tb_accountKey').val();
		data.secretKey = $('#tb_secretKey').val();
		data.userKey = $('#tb_userKey').val();
		data.page = cntChannel;
		if (channelKey != '' && channelKey != undefined) {
			url += '/' + channelKey;
		}
		var list_media = $(this).parent().find('ul');
		$.ajax({
